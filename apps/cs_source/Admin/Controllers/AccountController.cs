...
namespace Vinaday.Admin.Controllers
{
    ...
    public class AccountController : Controller
    {
        public AccountController()
            : this(new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new SecurityContext())))
        {
        }
        ...
        [Authorize]
        public bool UserValid(string password)
        {
        ...
        {
            var success = false;
            returnUrl = !string.IsNullOrEmpty(Server.UrlDecode(returnUrl)) ? Server.UrlDecode(returnUrl) : "/";
            if (model == null) return Json(null, JsonRequestBehavior.AllowGet);
            ...
            if (user != null)
            {
                ...
                //var ss = db.Roles.ToList();
                //var role = (from r in db.Roles where r.Name.Contains("Master Admin") select r).FirstOrDefault();
            ...
            var jsonResult = Json(resutl, JsonRequestBehavior.AllowGet);
